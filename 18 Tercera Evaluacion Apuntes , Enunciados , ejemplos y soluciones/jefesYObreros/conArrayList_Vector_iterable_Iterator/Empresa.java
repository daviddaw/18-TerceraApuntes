...
public class Empresa /*implements Iterable<Empleado>*/{
...
/***************************************************************************************************************/
	...
	private Empleado validarCodigoEmpleado(String codigo) {
		...
		e=buscarStringListaForeach(codigo);
		return e;	
	...
	private Empleado buscarStringListaForeach(String codigo) {
		Empleado e = null;
	...
	public void listarTodo() {
		...
		for (int i = 0; i < listaEmpleados.size(); i++) {
			if (listaEmpleados.get(i) instanceof Obrero) {// todos los obreros
				oAux=(Obrero)listaEmpleados.get(i);
				if (oAux.getJefe()==null) {//obreros sin jefe
	...
	public void listarTodoConForeachIterable() {
		System.out.println("Con bucle foreach solo los empleados con mas de 2000$");
		for (Empleado empleado : listaEmpleados) {
			if (empleado.getSueldo()>2000) {
				if (empleado instanceof Obrero) {//si es obrero
					Obrero oAux = (Obrero)empleado;
					if (oAux.getJefe()==null) {
						System.out.println("Codigo "+oAux.getCodigo()+" Edad "+oAux.getEdad()+" Nombre"+oAux.getNombre()+" Sueldo "+oAux.getSueldo());
					} else {
						System.out.println(empleado.toString());
					}
				}else {//si no es obrero (es jefe)
					System.out.println(empleado.toString());
				}
			}
		}
...
/*		for (int i = 0; i < listaEmpleados.size(); i++) {
		...
		for (Empleado empleado : listaEmpleados) {
			System.out.println("antes de la subida"+empleado);
			nuevoSueldo = empleado.getSueldo()+empleado.getSueldo()*((double)porcentaje/100);
			if (empleado instanceof Jefe) {
				nuevoSueldo=nuevoSueldo+120.0;
				System.out.println(nuevoSueldo+" suma");
			}
			empleado.setSueldo(nuevoSueldo);
			System.out.println("despues de la subida"+empleado.toString());
	...
	public void obrerosDeJefe() {//con iterator while has next 
		Jefe jAux;
		String codigo;
		System.out.println("Inserta el codigo del jefe");
		codigo=in.next();
		...
		if (e==null) {
			...
			jAux.recorrerListaJefeMostrarNombreDeEmpleadosConWhileHasNext();	
			}else {
